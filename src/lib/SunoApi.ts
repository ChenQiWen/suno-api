...
import yn from 'yn';
import { sleep, waitForRequests } from '@/lib/utils';
...
class SunoApi {
  ...
  */
  ...
  private async getCaptcha(): Promise<string | null> {
    if (!await this.captchaRequired()) {
      return null;
    }
    const { getCaptchaToken } = await import('./SunoCaptcha');
    return getCaptchaToken(this.userAgent!, this.currentToken!, this.cookies, (newToken) => {
      this.currentToken = newToken;
    });
  }
